...
GLuint LoadTGACubemap(const char** filenames) // Loads TGA files to a cubemap and return ID.
{
	GLuint type = GL_RGBA;
	...
	for (int i = 0; i<6; ++i)
	{
		...
		TextureData* texturep = &texture;
		LoadTGATextureData(filenames[i], texturep);	// Loads A TGA File Into Memory
		if (texturep->bpp == 8)						// Was The TGA 8 Bits? Should be grayscale then.
		{
			type=GL_RED;			// If So Set The 'type' To GL_RED
		}
		if (texturep->bpp == 24)						// Was The TGA 24 Bits?
		{
			type=GL_RGB;			// If So Set The 'type' To GL_RGB
		}
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, type, texturep->width, texturep->height, 0, type, GL_UNSIGNED_BYTE, texturep[0].imageData);
