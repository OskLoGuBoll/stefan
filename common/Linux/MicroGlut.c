...
void glutLogicFunc(void (*func)(void))
{
	gLogic = func;
}
...
//			op = 1; 
			...
			case ButtonPress:
				...
				case Button4:
					gMouseFunc(GLUT_SCROLL_UP, GLUT_DOWN, event.xbutton.x, event.xbutton.y);break;
				case Button5:
					gMouseFunc(GLUT_SCROLL_DOWN, GLUT_DOWN, event.xbutton.x, event.xbutton.y);break;
				}
				break;
			case ButtonRelease:
...
void glutRepeatingTimer(int micros)
{
	TimerRec *t	= (TimerRec *)malloc(sizeof(TimerRec));
	t->arg = 0;
	t->time = micros + glutGet(GLUT_ELAPSED_TIME_U);
	t->repeatTime = micros;
	t->repeating = 1;
	t->func = NULL;
	t->next = gTimers;
